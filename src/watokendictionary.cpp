...
void WATokenDictionary::getToken(QString &string, int &subdict, int token)
{
    if(!subdict) {
        if(token >= dictStrings[0].size() && token < (dictStrings[0].size() + dictStrings[1].size())) {
            subdict = 1;
        } else if(token > dictStrings[0].size() + dictStrings[1].size()) {
            subdict = 2;
        }
    }
    ...
    if (token < 0) {
        ...
        string = dictStrings[subdict].at(token);
    }
}

int WATokenDictionary::dictSize(int n) const
{
    return dictStrings[n].size();
}
